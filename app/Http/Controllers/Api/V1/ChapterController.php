...
class ChapterController extends Controller
{
    ...
    public function show(Request $request, string $storySlug, string $chapterSlug): ChapterContentResource
    {
        ...
        // ── View count gating (3 layers) ──────────────────────────────
        // 1. Skip SSR: Next.js server pre-rendering should not inflate views
        // 2. Skip excluded IPs: Admin/internal traffic (same list as analytics)
        // 3. Skip bots: Crawlers with real browser User-Agents
        $ip = (string) $request->ip();
        $shouldCount = !$request->header('X-Secret-Key')
            && !in_array($ip, (array) config('analytics.tracking.exclude_ips', []), true)
            && !app(\App\Services\Analytics\Collector\BotDetector::class)->isBot(
                $request->header('X-Forwarded-User-Agent') ?? $request->userAgent() ?? ''
            );
