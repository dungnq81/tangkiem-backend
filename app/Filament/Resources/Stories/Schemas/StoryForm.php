...
class StoryForm
{
    ...
    private static function statisticsSection(): Section
    {
        return Section::make('Thống kê truyện')
            ...
            ->schema([
                Grid::make(4)->schema([
                    self::totalChaptersDisplay(),
                    self::totalWordCountDisplay(),
                    self::viewCountDisplay(),
                    self::favoriteCountDisplay(),
                ]),
                Grid::make(4)->schema([
                    self::viewCountDayDisplay(),
                    self::viewCountWeekDisplay(),
                    self::viewCountMonthDisplay(),
                    self::commentCountDisplay(),
                ]),
                Grid::make(3)->schema([
                    self::ratingDisplay(),
                    self::ratingCountDisplay(),
                    self::latestChapterTitleDisplay(),
                ]),
            ])
            ->columns(1);
    }
    ...
    private static function totalChaptersDisplay(): TextInput
    {
        return TextInput::make('total_chapters')
            ->label('Số chương')
            ->disabled()
            ->dehydrated(false);
    }

    private static function totalWordCountDisplay(): TextInput
    {
        return TextInput::make('total_word_count')
            ->label('Số từ')
            ->disabled()
            ->dehydrated(false);
    }
    ...
    private static function favoriteCountDisplay(): TextInput
    {
        return TextInput::make('favorite_count')
            ->label('Yêu thích')
            ->disabled()
            ->dehydrated(false);
    }

    private static function viewCountDayDisplay(): TextInput
    {
        return TextInput::make('view_count_day')
            ->label('Xem hôm nay')
            ->disabled()
            ->dehydrated(false);
    }

    private static function viewCountWeekDisplay(): TextInput
    {
        return TextInput::make('view_count_week')
            ->label('Xem tuần')
            ->disabled()
            ->dehydrated(false);
    }

    private static function viewCountMonthDisplay(): TextInput
    {
        return TextInput::make('view_count_month')
            ->label('Xem tháng')
            ->disabled()
            ->dehydrated(false);
    }
    ...
    private static function ratingDisplay(): TextInput
    {
        return TextInput::make('rating')
            ->label('Điểm đánh giá')
            ->disabled()
            ->dehydrated(false);
    }

    private static function ratingCountDisplay(): TextInput
    {
        return TextInput::make('rating_count')
            ->label('Số đánh giá')
            ->disabled()
            ->dehydrated(false);
    }

    private static function latestChapterTitleDisplay(): TextInput
    {
        return TextInput::make('latest_chapter_title')
