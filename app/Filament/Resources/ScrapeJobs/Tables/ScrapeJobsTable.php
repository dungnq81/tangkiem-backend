...
class ScrapeJobsTable
{
    public static function configure(Table $table): Table
    {
        return $table
                ...
                ActionGroup::make([
                    Action::make('startScrape')
                        ...
                        ->action(function ($record) {
                            try {
                                $record->markScraping();
                                RunScrapeJob::dispatchSync($record);

                                Notification::make()
                                    ->title('Đã thu thập xong!')
                                    ->body('Kiểm tra kết quả ở trang chi tiết.')
                                    ->success()
                                    ->send();
                            } catch (\Throwable $e) {
            ...
            ->recordClasses(function ($record) {
                $totalItems = $record->items_count ?? 0;
                if ($totalItems === 0) {
                    return '';
                }
    ...
    private static function statusColumn(): TextColumn
    {
        return TextColumn::make('status')
            ...
            ->state(function ($record) {
                $totalItems = $record->items_count ?? $record->items()->count();

                if ($totalItems === 0) {
                    return $record->status;
                }

                $hasPending = $record->items()
                    ->whereIn('status', ['draft', 'selected'])
                    ->exists();

                $hasError = $record->items()
                    ->where('status', 'error')
                    ->exists();
    ...
    private static function itemsCountColumn(): TextColumn
    {
        return TextColumn::make('items_count')
            ...
            ->counts('items')
            ->state(function ($record) {
                $total = $record->items_count ?? $record->items()->count();
                if ($total === 0) {
                    return '0';
                }

                $imported = $record->items()
                    ->whereIn('status', ['imported', 'merged'])
                    ->count();
            ...
            ->tooltip(function ($record) {
                ...
                if ($totalItems > 0) {
                    ...
                    if ($importedCount === $totalItems) {
                        ...
                        $parts[] = "{$importedCount}/{$totalItems} imported";
                    }
                }
