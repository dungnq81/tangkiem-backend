...
class ScrapeJobProgressWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $stats = [
            $this->statusStat($record)->extraAttributes($compact),
        ...
        if ($record->isChapterType() && $record->hasDetailConfig()) {
            $stats[] = $this->detailStat($record)->extraAttributes($compact);
        }
    ...
    private function statusStat(ScrapeJob $record): Stat
    {
        ...
        $statusConfig = match ($record->status) {
            ScrapeJob::STATUS_DRAFT    => ['label' => '📝 Nháp', 'color' => 'gray', 'desc' => $this->getDraftDescription($record)],
            ScrapeJob::STATUS_SCRAPING => ['label' => '⏳ Đang thu thập...', 'color' => 'warning', 'desc' => $scrapingDesc],
            ScrapeJob::STATUS_SCRAPED  => ['label' => '✅ Đã thu thập', 'color' => 'success', 'desc' => $this->getScrapedDescription($record)],
            ScrapeJob::STATUS_IMPORTING => ['label' => '📥 Đang import...', 'color' => 'primary', 'desc' => 'Đang ghi vào database'],
            ScrapeJob::STATUS_DONE     => ['label' => '🎉 Hoàn tất', 'color' => 'success', 'desc' => $this->getDoneDescription($record)],
    ...
    private function getDraftDescription(ScrapeJob $record): string
    {
        ...
        $importedCount = $record->items()
            ->whereIn('status', ['imported', 'merged'])
            ->count();
    ...
    private function getDoneDescription(ScrapeJob $record): string
    {
        $imported = $record->items()->where('status', 'imported')->count();
        $merged = $record->items()->where('status', 'merged')->count();
    ...
    private function getScrapedDescription(ScrapeJob $record): string
    {
        if (! $record->isChapterType() || ! $record->hasDetailConfig()) {
            // Check if all items are already imported
            $draftCount = $record->items()->where('status', 'draft')->count();
        ...
        // (detail_status gets reset on re-scrape, but item statuses persist)
        $totalItems = $record->items()->count();
        $importedCount = $record->items()->whereIn('status', ['imported', 'merged'])->count();
    ...
    private function detailStat(ScrapeJob $record): Stat
    {
        ...
        // Also count imported items for accurate status
        $importedCount = $record->items()->whereIn('status', ['imported', 'merged'])->count();
