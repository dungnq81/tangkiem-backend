...
class ScrapeJobProgressWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        // Show detail stat for chapter jobs with detail_config AND for chapter_detail
        if (($record->isChapterType() && $record->hasDetailConfig()) || $record->isChapterDetailType()) {
    ...
    {
        // chapter_detail: content is already embedded in each crawled page
        if ($record->isChapterDetailType()) {
            $totalItems = array_sum($itemCounts);
            $importedCount = ($itemCounts['imported'] ?? 0) + ($itemCounts['merged'] ?? 0);
            $selectedCount = $itemCounts['selected'] ?? 0;
            ...
            if ($selectedCount > 0) {
                return "Có {$selectedCount} chương sẵn sàng import";
            }
    ...
    private function progressStat(ScrapeJob $record): Stat
    {
        ...
        $isChapterDetail = $record->isChapterDetailType();

        // Chapter detail uses "chương" (chapter) not "trang" (page)
        $unit = $isChapterDetail ? 'chương' : 'trang';

        if ($record->status === ScrapeJob::STATUS_SCRAPING) {
            if ($isChapterDetail) {
                // During chain crawl: show current chapter count being crawled
                $value = "{$current} chương";
                $desc = 'Đang cào...';
            } else {
    ...
    private function detailStat(ScrapeJob $record, array $itemCounts): Stat
    {
        ...
        if ($fetched === 0 && $errors === 0) {
            $emptyDesc = $isChapterDetail
                ? 'Đang cào — nội dung sẽ có khi crawl xong'
                : 'Chưa fetch — chọn chương → bulk action "📖 Fetch nội dung"';
        ...
        if ($importedCount > 0 && $importedCount === $total) {
            ...
            if ($fetched > 0) {
                $contentLabel = $isChapterDetail ? 'có nội dung' : 'đã fetch';
                $fetchLabel = $importedCount > 0
                    ? "✅ {$fetched} {$contentLabel} ({$importedCount} imported)"
                    : "✅ {$fetched} {$contentLabel}";
                $parts[] = $fetchLabel;
            }
            if ($errors > 0) {
                $parts[] = "❌ {$errors} lỗi";
            ...
            if ($unfetched > 0) {
                $unfetchedLabel = $isChapterDetail ? 'không có nội dung' : 'chưa fetch';
                $parts[] = "⏳ {$unfetched} {$unfetchedLabel}";
            }
        }
