...
trait HasScrapeActions
{
    protected function startScrapeAction(): Actions\Action
    {
        return Actions\Action::make('startScrape')
            ->label(function () {
                // If chain crawl has resume data, clarify this starts fresh
                if ($this->record->isChapterDetailType()
                    && ! empty($this->record->detail_config['last_crawled_url'] ?? null)) {
                    return 'Thu thập từ đầu';
                }

                return 'Bắt đầu thu thập';
            })
            ->color('success')
            ...
            ->modalDescription(function () {
                if ($this->record->isChapterDetailType()
                    && ! empty($this->record->detail_config['last_crawled_url'] ?? null)) {
                    return 'Sẽ thu thập lại từ đầu. Dữ liệu cũ sẽ bị giữ lại nhưng tiến trình resume sẽ mất. Dùng nút "Tiếp tục thu thập" nếu muốn tiếp tục.';
                }
            ...
            ->action(function () {
                try {
                    // For chain crawl with resume data: clear it so we truly start fresh
                    if ($this->record->isChapterDetailType()
                        && ! empty($this->record->detail_config['last_crawled_url'] ?? null)) {
                        $config = $this->record->detail_config ?? [];
                        unset($config['last_crawled_url'], $config['chain_last_chapter_num']);
                        $this->record->update(['detail_config' => $config]);
                    }

                    $this->record->markScraping();
                    $this->dispatchScrapeJob($this->record);
    ...
    /**
     * Resume a chain crawl from the last saved position.
     * Only visible when chain has resume data (last_crawled_url).
     */
    protected function resumeChainAction(): Actions\Action
    {
        return Actions\Action::make('resumeChain')
            ->label('Tiếp tục thu thập')
            ->color('success')
            ->icon(Heroicon::OutlinedPlayPause)
            ->requiresConfirmation()
            ->modalHeading('Tiếp tục thu thập?')
            ->modalDescription(function () {
                $config = $this->record->detail_config ?? [];
                $chapterNum = $config['chain_last_chapter_num'] ?? '?';
                $url = Str::limit($config['last_crawled_url'] ?? '', 60);

                return "Tiếp tục từ chương {$chapterNum}. URL: {$url}";
            })
            ->visible(fn () => $this->record->isChapterDetailType()
                && ! $this->record->isSingleChapterMode()
            ...
            ->action(function () {
                try {
                    ...
                    Notification::make()
                        ->title('Đang tiếp tục thu thập')
                        ->body('Chain crawl sẽ tiếp tục từ chương đã dừng. Tiến trình sẽ tự cập nhật.')
                        ->success()
                        ->send();
        ...
        return Actions\Action::make('retryScrape')
            ...
            ->action(function () {
                try {
                    ...
                    // Clear chain resume state if present
                    if ($this->record->isChapterDetailType()) {
                        $config = $this->record->detail_config ?? [];
                        unset($config['last_crawled_url'], $config['chain_last_chapter_num']);
                        $resetData['detail_config'] = $config;
                    }

                    $this->record->update($resetData);
    ...
    /**
     * Dispatch RunScrapeJob to queue + ensure it gets processed immediately.
     */
    private function dispatchScrapeJob(ScrapeJob $record): void
    {
