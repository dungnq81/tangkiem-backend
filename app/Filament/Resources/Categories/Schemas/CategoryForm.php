...
class CategoryForm
{
    ...
    private static function childrenCountInput(): TextInput
    {
        return TextInput::make('children_count')
            ->label('Số thể loại con')
            ->numeric()
            ->default(0)
            ->disabled()
            ->dehydrated(false);
    ...
    private static function depthInput(): TextInput
    {
        return TextInput::make('depth')
            ->label('Cấp độ')
            ->numeric()
            ->disabled()
            ->dehydrated(false)
            ->helperText(new HtmlString('<span class="text-xs">Tự động tính theo cấp độ thể loại cha</span>'));
    }
}
