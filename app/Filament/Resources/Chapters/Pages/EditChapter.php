    ...
    protected function getAiCleanAction(): Action
    {
        return Action::make('ai_clean')
            ...
            ->action(function (array $data): void {
                set_time_limit(120); // AI calls may take time
                ...
                try {
                    ...
                    // Update form state only (NOT saved to DB yet)
                    // User reviews changes and clicks Save manually
                    $this->data['content']['content'] = $result['content'];

                    // Build detailed report
                    $diff = $result['charDiff'];
                    $diffLabel = $diff > 0 ? "Xóa **{$diff}** ký tự rác" : "Thay đổi **" . abs($diff) . '** ký tự';
                    $lines = [$diffLabel, '', '⚠️ *Nhấn **Lưu** để áp dụng thay đổi.*'];
                    foreach ($result['removals'] as $r) {
                        $lines[] = '- ' . $r;
                    }

                    Notification::make()
                        ->title('Đã dọn dẹp — chưa lưu')
