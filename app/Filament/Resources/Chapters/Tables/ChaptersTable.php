    ...
    public static function configure(Table $table): Table
    {
        return $table
            ->modifyQueryUsing(function ($query) {
                ...
                // Avoids loading meta_title, meta_description, scrape_url,
                // scrape_hash — saves memory and reduces row size.
                $query->select([
                    'id', 'story_id', 'chapter_number', 'sort_key',
                    'sub_chapter', 'volume_number', 'title', 'slug',
                    'word_count', 'view_count', 'is_published', 'is_vip',
                    'published_at', 'scheduled_at', 'created_at', 'deleted_at',
                    'scrape_source_id', // Used by aiClean bulk action
                ])
                ->with('story:id,title');

                $volumeNumber = request()->input('tableFilters.volume_number.value');
            ...
            ->columns([
                ...
                TextColumn::make('volume_number')
                    ...
                    ->getStateUsing(function (Chapter $record): ?HtmlString {
                        if ($record->volume_number === null || $record->volume_number < 1) {
                            return null;
                        }

                        $url = self::getVolumeFilterUrl($record->story_id, $record->volume_number);
                        $label = "Quyển {$record->volume_number}";
                ...
                TextColumn::make('sub_chapter')
                    ->label('Phần')
                    ->sortable()
                    ->alignCenter()
                    ->getStateUsing(function (Chapter $record): ?HtmlString {
                        if ($record->sub_chapter <= 0) {
                            return null;
                        }

                        $label = "Phần {$record->sub_chapter}";

                        return new HtmlString(sprintf(
                            '<span class="fi-badge inline-flex items-center rounded-md text-xs font-medium ring-1 ring-inset px-1.5 py-0.5" style="background-color: rgba(34, 197, 94, 0.1); color: rgb(34, 197, 94); border-color: rgba(34, 197, 94, 0.3);">%s</span>',
                            e($label)
                        ));
                    })
                    ->html()
                    ->placeholder('-')
                    ->toggleable(isToggledHiddenByDefault: true)
                    ->tooltip(fn (Chapter $record) => $record->sub_chapter > 0 ? "Phần {$record->sub_chapter}" : null),
                    ...
                    BulkAction::make('aiClean')
                        ...
                        ->form([
                            Select::make('mode')
                                ...
                                ->options([
                                    'patterns_only' => 'Patterns + Regex',
                                    'ai'            => 'Patterns + Regex + AI',
                                ])
                        ...
                        ->action(function (Collection $records, array $data): void {
                            set_time_limit(300); // Bulk AI clean may take time
                            ...
                            foreach ($records as $chapter) {
                                ...
                                if ($original !== $result) {
                                    ...
                                    $cleaned++;
                                    $totalDiff += mb_strlen($original) - mb_strlen($result);
                                } else {
                                    $skipped++;
                                }
                            }

                            $diffLabel = $totalDiff >= 0
                                ? '🗑️ Xóa: ' . number_format($totalDiff) . ' ký tự'
                                : '📝 Thay đổi: ' . number_format(abs($totalDiff)) . ' ký tự';

                            Notification::make()
                                ->title('Dọn dẹp hoàn tất')
                                ->body("✅ Dọn: {$cleaned} chương | ⏭️ Bỏ qua: {$skipped} | {$diffLabel}")
                                ->success()
                                ->send();
                        })
                        ->deselectRecordsAfterCompletion(),
            ...
            ->emptyStateHeading('Chọn truyện để xem chương')
            ->emptyStateDescription('Sử dụng bộ lọc "Chọn truyện" phía trên để hiển thị danh sách chương.')
