...
class AuthorForm
{
    ...
    private static function statusSection(): Section
    {
        return Section::make()
            ->schema([
                ...
                Grid::make(3)->schema([
                    self::storiesCountInput(),
                    self::totalViewsInput(),
                    self::totalChaptersInput(),
                ]),
            ])
            ->columns(1);
    }
    ...
    private static function metaTitleInput(): TextInput
    {
        return TextInput::make('meta_title')
            ->label('Meta Title')
            ->maxLength(60)
            ->helperText(new HtmlString('<span class="text-xs">Tối đa 60 ký tự. Để trống sẽ dùng tên tác giả</span>'));
    }
    ...
    private static function storiesCountInput(): TextInput
    {
        return TextInput::make('stories_count')
            ->label('Số truyện')
            ->numeric()
            ->default(0)
            ->disabled()
            ...
            ->afterStateHydrated(function (TextInput $component, $record) {
                if ($record) {
                    $component->state($record->stories()->count());
                }
            })
            ->helperText(new HtmlString('<span class="text-xs">Tự động cập nhật</span>'));
    }

    private static function totalViewsInput(): TextInput
    {
        return TextInput::make('total_views')
            ->label('Tổng lượt xem')
            ->numeric()
            ->default(0)
            ->disabled()
            ...
            ->afterStateHydrated(function (TextInput $component, $record) {
                if ($record) {
                    $component->state((int) $record->stories()->sum('view_count'));
                }
            })
            ->helperText(new HtmlString('<span class="text-xs">Tự động cập nhật</span>'));
    }

    private static function totalChaptersInput(): TextInput
    {
        return TextInput::make('total_chapters')
            ...
            ->afterStateHydrated(function (TextInput $component, $record) {
                if ($record) {
                    $component->state((int) $record->stories()->sum('total_chapters'));
                }
            })
            ->helperText(new HtmlString('<span class="text-xs">Tự động cập nhật</span>'));
    }
