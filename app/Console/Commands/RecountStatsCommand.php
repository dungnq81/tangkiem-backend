...
class RecountStatsCommand extends Command
{
    protected $signature = 'app:recount-stats
                            {model? : Specific model to recount (tags, categories, stories, chapters, comments, authors, bookmarks)}
    ...
    public function handle(): int
    {
        ...
        $recounters = [
            ...
            'bookmarks'  => fn () => $this->recountBookmarkFavoriteCount($isDry),
        ];
    ...
    /**
     * Recount stories.favorite_count from bookmarks table.
     */
    protected function recountBookmarkFavoriteCount(bool $isDry): void
    {
        $this->info('📊 Recounting stories.favorite_count...');

        $prefix = DB::getTablePrefix();

        if ($isDry) {
            $mismatched = DB::select("
                SELECT s.id, s.title, s.favorite_count AS current,
                       COALESCE(bk.cnt, 0) AS actual
                FROM {$prefix}stories s
                LEFT JOIN (
                    SELECT story_id, COUNT(*) AS cnt
                    FROM {$prefix}bookmarks
                    GROUP BY story_id
                ) bk ON bk.story_id = s.id
                WHERE s.favorite_count != COALESCE(bk.cnt, 0)
                AND s.deleted_at IS NULL
                LIMIT 50
            ");
        ...
        $updated = DB::update("
            ...
            SET s.favorite_count = COALESCE(bk.cnt, 0)
            WHERE s.favorite_count != COALESCE(bk.cnt, 0)
            AND s.deleted_at IS NULL
        ");
