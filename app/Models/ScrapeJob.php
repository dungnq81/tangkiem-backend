...
class ScrapeJob extends Model
{
    ...
    /**
     * Whether this chapter_detail job is in single-chapter mode.
     * Single mode: scrape only the target_url page (1 chapter), then stop.
     */
    public function isSingleChapterMode(): bool
    {
        return $this->isChapterDetailType()
            && (bool) ($this->detail_config['single_chapter_mode'] ?? false);
    }
    ...
    /**
     * Resolve detail config: merge source-level content cleaning into job detail_config.
     *
     * Source-level remove_selectors/remove_text_patterns serve as global defaults
     * for ALL jobs of this source. Job-level values are appended (not overwritten).
     *
     * @return array The merged detail config
     */
    public function resolveDetailConfig(): array
    {
        $config = $this->detail_config ?? [];
        $source = $this->source;
        ...
        // Merge remove_selectors: source (global) + job (specific)
        $sourceSelectors = trim($source->remove_selectors ?? '');
        $jobSelectors = trim($config['remove_selectors'] ?? '');

        if ($sourceSelectors !== '' || $jobSelectors !== '') {
            $config['remove_selectors'] = implode("\n", array_filter([
                $sourceSelectors,
                $jobSelectors,
            ]));
        }

        // Merge remove_text_patterns: source (global) + job (specific)
        $sourcePatterns = trim($source->remove_text_patterns ?? '');
        $jobPatterns = trim($config['remove_text_patterns'] ?? '');

        if ($sourcePatterns !== '' || $jobPatterns !== '') {
            $config['remove_text_patterns'] = implode("\n", array_filter([
                $sourcePatterns,
                $jobPatterns,
            ]));
        }
