...
class AiService
{
    ...
    /** Default AI provider when none is configured in settings. */
    ...
    /**
     * Check if the current AI provider supports Google Search grounding.
     *
     * Only Gemini supports this feature. When useSearch is requested with
     * a non-Gemini provider, AiService forces a fallback to Gemini — which
     * wastes Gemini quota. Callers should check this before setting useSearch.
     */
    public function supportsSearchGrounding(): bool
    {
        $provider = Setting::get('ai.provider') ?: self::DEFAULT_PROVIDER;
    ...
    /**
     * Generate image with 3-tier fallback chain.
     *
     * Strategy:
     * 1. Imagen Predict API (dedicated :predict endpoint — most reliable, requires billing)
     * 2. IMAGE_FALLBACK_PROVIDERS (currently: AI Magicx — Flux, DALL-E 3, Stable Diffusion)
     * 3. Gemini GenerateContent (last resort — free tier, often fails/timeouts)
     *
    ...
    public function callImage(
        ...
        // ── Tier 1: Imagen Predict API (dedicated, most reliable) ──
        $predictModel = config('ai.imagen.predict_model');
        if ($predictModel && $config && ! empty($config['api_key'])) {
            try {
                return $this->callImagenPredict($prompt, $aspectRatio, $config, $predictModel);
            } catch (\Throwable $e) {
                $errors[] = "Imagen Predict: {$e->getMessage()}";
                Log::warning('AI callImage: Imagen Predict failed, trying AiMagicx', [
                    'model' => $predictModel,
                    'error' => $e->getMessage(),
                ]);
            }
        ...
        // ── Tier 2: External fallback providers (AiMagicx — Flux, DALL-E 3, SD) ──
        ...
        $errors[] = 'AiMagicx fallback: no providers succeeded';

        // ── Tier 3: Gemini GenerateContent (last resort — free tier, often unreliable) ──
        if ($config && ! empty($config['api_key'])) {
            try {
                return $this->callGeminiImage($prompt, $aspectRatio, $config);
            } catch (\Throwable $e) {
                $errors[] = "Gemini GenerateContent: {$e->getMessage()}";
                Log::warning('AI callImage: Gemini GenerateContent also failed', [
                    'error' => $e->getMessage(),
                ]);
            }
        } else {
            Log::info('AI callImage: Gemini not configured');
        }

        // All providers failed
        $chain = implode(' → ', $errors);
        throw new \RuntimeException(
            "Tạo ảnh thất bại.\n🔗 {$chain}\n💡 Kiểm tra API key, billing hoặc thử lại sau."
        );
    }

    /**
     * Generate image via Imagen Predict API (dedicated image endpoint).
     *
     * Uses the :predict endpoint which is purpose-built for image generation.
     * More reliable than :generateContent because:
     * - Dedicated quota (not shared with text generation)
     * - Always returns image or clear error (never falls back to text)
     * - Production-ready model (Imagen 4)
     *
     * Requires billing enabled on Google AI Studio.
     *
     * @return string Base64-encoded image data
     */
    protected function callImagenPredict(
        string $prompt,
        string $aspectRatio,
        array $config,
        string $model,
    ): string {
        $url = "{$config['base_url']}/models/{$model}:predict?key={$config['api_key']}";

        Log::debug('AI callImage starting (Imagen Predict)', [
            'model'        => $model,
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            $lastResponse = Http::connectTimeout(10)->timeout(60)->post($url, $payload);
            ...
            if ($lastResponse->status() === 429 && $attempt < $maxAttempts) {
                ...
                Log::warning("AI Imagen Predict: Rate limited (429), waiting {$waitSeconds}s before retry {$attempt}/{$maxAttempts}");
                sleep($waitSeconds);
            ...
            // Non-retryable error
            throw new \RuntimeException(
                $this->formatApiError('Imagen Predict', $lastResponse->status(), $lastResponse->body())
            );
        }
        ...
        foreach ($predictions as $prediction) {
            if (! empty($prediction['bytesBase64Encoded'])) {
                Log::info('AI callImage: Imagen Predict succeeded', ['model' => $model]);

                return $prediction['bytesBase64Encoded'];
            }
        }

        throw new \RuntimeException("Imagen Predict ({$model}): API không trả về dữ liệu ảnh. Kiểm tra billing đã enable chưa.");
    }

    /**
     * Generate image via Gemini GenerateContent (fallback image provider).
     *
     * Uses the standard generateContent endpoint with responseModalities: ['Image'].
     * Less reliable than Imagen Predict: text-first endpoint, shared quota, may return
     * text instead of image. Works on free tier.
     *
     * @return string Base64-encoded image data
     */
    protected function callGeminiImage(string $prompt, string $aspectRatio, array $config): string
    {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            $lastResponse = Http::connectTimeout(10)->timeout(90)->post($url, $payload);
        ...
        throw new \RuntimeException("Gemini Image ({$imageModel}): Không trả về dữ liệu ảnh. Model có thể trả text thay vì ảnh.");
    }
    ...
    /**
     * Format user-friendly error message for text/JSON generation failures.
     *
     * Maps common error scenarios to Vietnamese messages with actionable guidance.
     */
    protected function formatUserFriendlyError(string $provider, string $model, string $rawError): string
    {
        $lower = strtolower($rawError);
        $prefix = "⚠️ AI ({$provider}/{$model})";

        // Rate limit / Quota
        if (str_contains($lower, '429') || str_contains($lower, 'rate limit') || str_contains($lower, 'quota')) {
            return "{$prefix}: Vượt giới hạn API.\n💡 Đợi vài phút hoặc đổi provider trong Cài đặt AI.";
        }

        // Auth / API key
        if (str_contains($lower, '401') || str_contains($lower, 'unauthorized') || str_contains($lower, 'invalid api key')) {
            return "{$prefix}: API key không hợp lệ hoặc đã hết hạn.\n💡 Kiểm tra API key trong .env.";
        }

        // Billing / Payment
        if (str_contains($lower, '402') || str_contains($lower, 'billing') || str_contains($lower, 'payment')) {
            return "{$prefix}: Cần kích hoạt billing cho API.\n💡 Bật billing trên Google AI Studio hoặc provider dashboard.";
        }

        // Forbidden
        if (str_contains($lower, '403') || str_contains($lower, 'forbidden') || str_contains($lower, 'permission')) {
            return "{$prefix}: Không có quyền truy cập API.\n💡 Kiểm tra quyền API key hoặc model có khả dụng không.";
        }

        // Timeout
        if (str_contains($lower, 'timeout') || str_contains($lower, 'timed out')) {
            return "{$prefix}: Hết thời gian chờ (timeout).\n💡 Thử lại hoặc chọn model nhanh hơn.";
        }

        // Server error
        if (str_contains($lower, '500') || str_contains($lower, '502') || str_contains($lower, '503') || str_contains($lower, 'server error')) {
            return "{$prefix}: Lỗi server phía API.\n💡 Thử lại sau ít phút.";
        }

        // Content safety / blocked
        if (str_contains($lower, 'safety') || str_contains($lower, 'blocked') || str_contains($lower, 'content filter')) {
            return "{$prefix}: Nội dung bị chặn bởi bộ lọc an toàn.\n💡 Thử prompt khác hoặc sửa nội dung đầu vào.";
        }

        // Network
        if (str_contains($lower, 'network') || str_contains($lower, 'connection') || str_contains($lower, 'could not resolve')) {
            return "{$prefix}: Lỗi kết nối mạng.\n💡 Kiểm tra internet hoặc thử lại.";
        }

        // Default: show raw error with context
        $truncated = Str::limit($rawError, 150);
    ...
    {
        $provider = $provider ?: (Setting::get('ai.provider') ?: self::DEFAULT_PROVIDER);
    ...
    protected function callWithRetry(
        ...
        $errorInfo = $this->formatUserFriendlyError($provider, $model, $errorMsg);
