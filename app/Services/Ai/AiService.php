...
use App\Services\Ai\Client\AiClientInterface;
...
class AiService
{
     ...
     */
    private const FALLBACK_PROVIDERS = ['gemini', 'groq', 'cerebras', 'deepseek', 'openai', 'grok', 'anthropic', 'abacus', 'blackbox'];
    ...
    /**
     * Generate image via Gemini Nano Banana (gemini-2.5-flash-image).
     *
    ...
    public function callImage(
        ...
        if (! $config || empty($config['api_key'])) {
            throw new \RuntimeException('Gemini API key is required for image generation.');
        }
        ...
        Log::debug('AI callImage starting', [
            'model'        => $imageModel,
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            ...
            // Non-retryable error — log full body, throw clean message
            Log::error('AI callImage failed', [
                'status' => $lastResponse->status(),
                'body'   => $lastResponse->body(),
            ]);

            throw new \RuntimeException(
                $this->formatApiError('Image', $lastResponse->status(), $lastResponse->body())
            );
        }
        ...
        Log::error('AI callImage: No image data in response', ['body' => $lastResponse->body()]);
        throw new \RuntimeException('Gemini Image không trả về dữ liệu ảnh. Vui lòng thử lại.');
    }
     ...
     *
     * Retry strategy:
     * - Attempt up to MAX_RETRIES with the primary provider
     * - On 429 (rate limit): extract "retry in Xs" delay, wait that long
     * - On other errors: exponential backoff (1s, 2s, 3s)
     * - If all retries fail: try fallback providers (skip if useSearch, Gemini-only)
     */
    protected function callWithRetry(
        ...
        // Try fallback providers (skip if useSearch — Gemini-only feature)
        if (! $useSearch) {
        ...
        throw new \RuntimeException(
            'AI call failed after ' . self::MAX_RETRIES . ' attempts: ' . $lastError?->getMessage()
        );
    }
     ...
     *
     * @param  AiResponse|null  &$result  Will be set if a call succeeds
     * @return \Throwable|null Last error if all attempts failed
     */
    private function attemptCalls(
        ...
        for ($attempt = 1; $attempt <= self::MAX_RETRIES; $attempt++) {
            try {
                ...
                $result = $client->chat($systemPrompt, $userPrompt, $options);
