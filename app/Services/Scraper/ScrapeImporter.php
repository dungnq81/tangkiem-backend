...
class ScrapeImporter
{
    ...
    public function importSelected(ScrapeJob $job): array
    {
        ...
        // Sequential numbering: start from existing max chapter_number + 1
        ...
        if ($sequentialNumbering && $isChapterJob && $job->parent_story_id) {
            $maxExisting = Chapter::where('story_id', $job->parent_story_id)
                ->max('sort_key');
            $sequentialCounter = $maxExisting ? (int) ceil((float) $maxExisting) : 0;
        }
        ...
        foreach ($items as $item) {
            try {
                ...
                if ($isChapterJob) {
                    Chapter::withoutEvents(function () use ($item, $job, &$results, &$storyIdsNeedingNavRebuild, $sequentialNumbering, &$sequentialCounter) {
                        $outcome = $this->importItem($item, $job, $sequentialNumbering ? ++$sequentialCounter : null);

                        if ($outcome === 'merged') {
                            $item->update(['status' => ScrapeItem::STATUS_MERGED, 'error_message' => null]);
                            $results['merged']++;

                            // Reclaim the sequential number — merged items keep their existing chapter_number,
                            // so the counter should not advance for them (avoids gaps like 1,2,5,6)
     ...
     *
     * @param  int|null  $sequentialNumber  When set, overrides chapter_number with this value (sequential numbering mode).
    ...
    public function importItem(ScrapeItem $item, ScrapeJob $job, ?int $sequentialNumber = null): string
    {
        return match ($job->entity_type) {
            ...
            ScrapeJob::ENTITY_CHAPTER, ScrapeJob::ENTITY_CHAPTER_DETAIL => $this->importChapter($item, $job, $sequentialNumber),
        };
    }
     ...
     *
     * @param  int|null  $sequentialNumber  When set, overrides chapter_number (sequential numbering mode for volume-based stories).
     */
    protected function importChapter(ScrapeItem $item, ScrapeJob $job, ?int $sequentialNumber = null): string
    {
        $data = $item->raw_data;
        $title = trim($data['title'] ?? '');
