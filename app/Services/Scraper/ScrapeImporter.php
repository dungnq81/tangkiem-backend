...
class ScrapeImporter
{
    ...
    public function importSelected(ScrapeJob $job): array
    {
        ...
        // Only mark done when ALL items have been processed (no selected or draft remaining)
        $remainingSelected = $job->items()->where('status', ScrapeItem::STATUS_SELECTED)->count();
        $remainingDraft = $job->items()->where('status', ScrapeItem::STATUS_DRAFT)->count();
    ...
    protected function importChapter(ScrapeItem $item, ScrapeJob $job): string
    {
        ...
        // Volume/part support (DB default: 1)
        $volumeNumber = isset($data['volume_number']) ? (int) $data['volume_number'] : 1;

        // Sub-chapter support (e.g., 15.1, 15.2) — DB default: 0 (main chapter)
        $subChapter = isset($data['sub_chapter']) ? (int) $data['sub_chapter'] : 0;
        ...
        if ($existing) {
            ...
            return DB::transaction(function () use ($existing, $title, $volumeNumber, $subChapter, $data, $storyTitle, $chapterNumber) {
                // Smart merge: update content/title without overwriting existing data
                $mergeData = [
                    'title' => $title ?: null,
                ...
                if ($content) {
                    ...
                    if (empty($existing->meta_description)) {
                        $updates['meta_description'] = Str::limit($singleLine, 155, '...');
                    }
                ...
                if (empty($existing->meta_title)) {
                    ...
                    $existing->update(['meta_title' => $metaTitle]);
                }
