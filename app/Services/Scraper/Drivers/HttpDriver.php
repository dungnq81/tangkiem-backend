...
class HttpDriver implements DriverInterface
{
    ...
    /**
     * Configure a pool PendingRequest with standard HttpDriver options.
     *
     * Centralizes: browser headers, timeout, retry, curl options,
     * SSL bypass and DNS resolution for local dev — so pool requests
     * behave identically to fetchHtml().
     *
     * @param  array<string, ?string>  $dnsCache  Shared DNS cache (by reference for mutation)
     * @return \Illuminate\Http\Client\PendingRequest  Ready for ->get()
     */
    public function createPoolRequest(
        ...
        if (app()->environment('local')) {
            ...
            if ($url) {
                $host = parse_url($url, PHP_URL_HOST) ?: '';
                if ($host) {
                    if (! array_key_exists($host, $dnsCache)) {
                        $dnsCache[$host] = $this->resolveHostViaGoogle($host);
                    }
                    $ip = $dnsCache[$host];
                    if ($ip) {
