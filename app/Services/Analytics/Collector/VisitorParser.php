...
class VisitorParser
{
     ...
     *
     * @param string|null $referrerUrl  Full Referer header value
     * @param string|null $currentHost  Current site's host (to detect self-referral)
     * @return array{domain: string|null, type: string}
     */
    public function parseReferrer(?string $referrerUrl, ?string $currentHost = null): array
    {
        ...
        if ($currentHost) {
            $currentNormalized = strtolower(preg_replace('/^www\./', '', $currentHost));
            if ($domain === $currentNormalized || str_ends_with($domain, '.' . $currentNormalized)) {
                return ['domain' => null, 'type' => 'direct'];
            }
        }
    ...
    private function detectBrowser(string $ua): ?string
    {
        ...
        // Many browsers include "Safari" or "Chrome" in their UA string.
        $browsers = [
            'Edge'      => '/Edg(?:e|A|iOS)?\\//i',
            'Opera'     => '/(?:OPR|Opera)\\//i',
            'Brave'     => '/Brave\\//i',
