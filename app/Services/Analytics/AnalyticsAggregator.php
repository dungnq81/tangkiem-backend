...
class AnalyticsAggregator
{
    ...
    private function calculateStats(
        ...
        $core = (clone $query)
            ...
            ->selectRaw('COUNT(DISTINCT session_hash) as unique_visitors')
            ->selectRaw('SUM(CASE WHEN device_type = ? THEN 1 ELSE 0 END) as desktop_views', ['desktop'])
            ->selectRaw('SUM(CASE WHEN device_type = ? THEN 1 ELSE 0 END) as mobile_views', ['mobile'])
            ->selectRaw('SUM(CASE WHEN device_type = ? THEN 1 ELSE 0 END) as tablet_views', ['tablet'])
            ->selectRaw('SUM(CASE WHEN is_bot = 1 THEN 1 ELSE 0 END) as bot_views')
            ->first();

        if (!$core || (int) $core->total_views === 0) {
        ...
        $referrers = $this->fetchBreakdown(
            $query, 'referrer_domain', 'referrer_type', 20,
            fn ($r) => ['domain' => $r->col1, 'type' => $r->col2, 'count' => (int) $r->count]
        );
