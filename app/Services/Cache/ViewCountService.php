...
/**
 * ViewCountService — GLOBAL view count tracking (all FE domains combined).
 *
 * Manages denormalized view counts on stories/chapters tables:
 * - Increment views: cached in Redis (fast, no DB write per request)
 * - Periodic sync: batch push from cache to DB (runs every 5 min via schedule)
 *
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ SCOPING: View counts are INTENTIONALLY GLOBAL                  │
 * │                                                                 │
 * │ stories.view_count / chapters.view_count = ALL domains combined │
 * │ Used for: public rankings, story listings, "popular" sections   │
 * │                                                                 │
 * │ For PER-SITE view data, query daily_analytics table instead:    │
 * │ → daily_analytics has api_domain_id column                      │
 * │ → Tracked by TrackPageVisit middleware → AnalyticsCollector      │
 * └─────────────────────────────────────────────────────────────────┘
 *
