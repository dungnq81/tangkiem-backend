...
Route::prefix('v1')->group(function () {
    ...
    // ═══════════════════════════════════════════════════════════════
    // Story Reviews — cache 5 minutes (public, global across all sites)
    // ═══════════════════════════════════════════════════════════════
    Route::middleware(['api.domain:stories', 'cacheResponse:300'])->group(function () {
        Route::get('stories/{id}/reviews', [StoryController::class, 'reviews'])
            ->name('api.v1.stories.reviews')
            ->where('id', '[0-9]+');
    });

    // ═══════════════════════════════════════════════════════════════
    // Story User Status — NOT cached (user-specific, auth required)
    // Returns bookmark/rating/history status in one call
    // ═══════════════════════════════════════════════════════════════
    Route::middleware(['api.domain:user', 'auth:sanctum', 'doNotCacheResponse'])->group(function () {
        Route::get('stories/{id}/user-status', [StoryController::class, 'userStatus'])
            ->name('api.v1.stories.userStatus')
            ->where('id', '[0-9]+');
    });
    ...
    // ═══════════════════════════════════════════════════════════════
    // Rankings API — cache 5 minutes, consolidated to one route
    // ═══════════════════════════════════════════════════════════════
    Route::middleware(['api.domain:rankings', 'cacheResponse:300'])->group(function () {
        Route::get('rankings/{period}', [RankingController::class, 'show'])
            ->name('api.v1.rankings.show')
            ->where('period', 'daily|weekly|monthly|all-time');
    });
    ...
    Route::middleware(['api.domain:user', 'auth:sanctum', 'doNotCacheResponse'])->group(function () {
        ...
        Route::patch('user', [UserController::class, 'updateProfile'])
            ->name('api.v1.user.updateProfile');
